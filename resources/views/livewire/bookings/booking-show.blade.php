...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if ($booking->isConfirmed() && auth()->user()?->can('payments.view'))
                <x-ui.button size="sm" variant="secondary" :href="route('payments.booking', $booking)" wire:navigate>Payments</x-ui.button>
            @endif
            @if ($booking->isConfirmed() && auth()->user()?->can('documents.view'))
                <x-ui.button size="sm" variant="secondary" :href="route('documents.booking', $booking)" wire:navigate>Documents</x-ui.button>
            @endif
            ...
            @endif
            @can('attributePartners', $booking)
                <x-ui.button size="sm" variant="secondary" :href="route('bookings.partners', $booking)" wire:navigate>Partners</x-ui.button>
            @endcan
            @if (auth()->user()?->can('commission.view'))
                <x-ui.button size="sm" variant="secondary" :href="route('bookings.commission', $booking)" wire:navigate>Commission</x-ui.button>
            @endif
