<div class="space-y-6">
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <x-ui.card title="Last payment" class="lg:col-span-2">
            @if ($summary['last_payment_date'])
                <div class="flex items-baseline justify-between">
                    <p class="text-2xl font-semibold tabular-nums">₹{{ number_format((float) $summary['last_payment_amount'], 2) }}</p>
                    <p class="text-sm text-(--content-muted)">on {{ \Illuminate\Support\Carbon::parse($summary['last_payment_date'])->format('d M Y') }}</p>
                </div>
                @if (\Illuminate\Support\Facades\Route::has('portal.payments.index'))
                    <x-ui.button variant="secondary" size="sm" class="mt-3" :href="route('portal.payments.index')" wire:navigate>View payment history</x-ui.button>
                @endif
            @else
                <p class="text-sm text-(--content-muted)">No payments recorded yet.</p>
            @endif
        </x-ui.card>
